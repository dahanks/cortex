...
def match(terms,mem):
    for t in terms:
        if t[0][0]=='!':
            if query(mem,t[1],t[2]): return False
        elif not query(mem,t[1],t[2]): return False
    return True

def getActions(lm,context,rules):
    for r in rules[context]:
        if r[0]==[['True']] or match(r[0],lm):
            if r[1][0][0]=='perform':
                return getActions(lm,r[1][0][1],rules)
            else:
                return r[1]
...
class MySpecialWorker(Worker):
    ...
    def on_message(self, frame):
        ...
        while not done:
            for action in getActions(self.lm,'init',self.rules):
                if action[0]=='nil':
                    done=True
                    break
                if action[0]=='post':
